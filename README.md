...
- `dshm` which is set up as an `emptyDir` and is used to store the cached results of the authentication. This volume may already be present on the pod if the "shared memory" feature was enabled for the notebook container. If so, the injector will avoid adding this volume
